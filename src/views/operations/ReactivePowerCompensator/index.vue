 ...
 * @fileName: index.vue
 * @filePath: src/views/operations/ReactivePowerCompensator/index.vue
 * @description: 无功补偿器
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await reactivePowerCompensatorApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await reactivePowerCompensatorApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await reactivePowerCompensatorApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
