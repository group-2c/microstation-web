...
<script setup>
  ...
  import { dict_temperature_manufacturers } from "_utils/dictionary"
  ...
  const _getTableList = async () => {
    ...
    try {
      ...
      dataCenter.value.tableList = res.data.content.map(item => {
        item.manufacturer_name = dict_temperature_manufacturers.find(x => x.key === item.manufacturer)?.value
        return item
      })
