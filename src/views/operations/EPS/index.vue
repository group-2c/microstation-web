/*
 * @author: zzp
 * @date: 2023-09-05 15:22:39
 * @fileName: index.vue
 * @filePath: src/views/operations/EPS/index.vue
 * @description: EPS电源
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await epsApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await epsApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await epsApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
