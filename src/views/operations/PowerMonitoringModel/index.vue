...
<script setup>
  ...
  const constColumns = [
    ...
    { title: "操 作", dataIndex: "operation", align: "center", fixed: "right" }
  ]
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await powerMonitoringApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await powerMonitoringApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await powerMonitoringApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
