 ...
 * @fileName: index.vue
 * @filePath: src/views/operations/DoublePowerController/index.vue
 * @description: 双电源控制器 
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await doublePowerControllerApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await doublePowerControllerApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await doublePowerControllerApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
