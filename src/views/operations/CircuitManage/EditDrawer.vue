<template>
  <a-drawer
    ...
    class="editDrawer"
  > 
    <a-form ref="formRef" :model="dataCenter.record" :label-col="{ span: 8 }">
      <a-row :gutter="30"> 
        <a-col :span="12">
          <a-form-item label="配电图名称" name="name" :rules="[{ required: true }]">
        ...
        <a-col :span="12">
          <a-form-item label="SVG上传" name="fileName" :rules="[{ required: true, message: '请上传SVG图' }]">
            <div class="fileArea">
...
const handleOk = async () => {
  _validateForm(async () => {
    ...
    try {
      ...
      if (!values.id) {
        await circuitManageApi.create(values)
        message.success("新增成功！")
      } else {
        await circuitManageApi.updateById(values.id, values)
