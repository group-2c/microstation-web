 ...
 * @fileName: index.vue
 * @filePath: src/views/operations/DieselEngine/index.vue
 * @description: 柴油发动机
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await dieselEngineApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await dieselEngineApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await dieselEngineApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
