 ...
 * @fileName: index.vue
 * @filePath: src/views/operations/TransformerProtectionDevice/index.vue
 * @description: 变压器保护装置
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await transformerProtectionDeviceApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await transformerProtectionDeviceApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
