 ...
 * @fileName: index.vue
 * @filePath: src/views/operations/TelemetryUnit/index.vue
 * @description: 遥信遥测单元
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  import telemetryUnitApi from "_api/telemetryUnit"
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await telemetryUnitApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await telemetryUnitApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await telemetryUnitApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
