/*
 * @author: zzp
 * @date: 2023-09-05 15:11:52
 * @fileName: index.vue
 * @filePath: src/views/operations/DryTemperatureController/Inverter/index.vue
 * @description: 干变温控器
 */
<template>
  <div class="parcel">
...
<script setup>
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await dryTemperatureControllerApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await dryTemperatureControllerApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await dryTemperatureControllerApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
