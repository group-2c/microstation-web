...
<script setup>
  ...
  const constColumns = [
    ...
    { title: "组织机构名称", dataIndex: "name", align: "left" },    
  ...
  const _getTableList = async () => {
    ...
    try {
      const res = await organizationApi.getByPage(data)
  ...
  const batchDelete = () => {
    ...
    const _deleteData = async () => {
      ...
      try {
        ...
        await organizationApi.deleteManyById(selectedRowKeys)
      } catch(err) {
  ...
  const handleDeleteItem = async row => {
    dataCenter.value.loading = true
    try {
      await organizationApi.deleteById(row.id)
    } catch(err) {
      message.error("删除失败: " + err)
    } finally {
      dataCenter.value.loading = false
