/*
 * @author: zzp
 * @date: 2023-12-29 11:14:05
 * @fileName: index.vue
 * @filePath: src/views/historicalStatistics/LineHistorical/index.vue
 * @description: 线路保护装置历史数据
 */
<template>
...
import { lineProtectiveApi } from "_api/historicalStatistics"
import dayjs from "dayjs"
import controllerApi from "_api/controller"
import lineProtectiveDeviceApi from "_api/lineProtectiveDevice"
...
const columns = ref([
  ...
  { title: "频 率", dataIndex: "frequency", align: "left", width: 120 },
  ...
  { title: "有功功率", dataIndex: "activePower", align: "left", width: 120 },
  { title: "无功功率", dataIndex: "reactivePower", align: "left", width: 120 },
  { title: "功率因数", dataIndex: "powerFactor", align: "left", width: 120 }
])
...
const _getDeviceList = async () => {
  try {
    const res = await lineProtectiveDeviceApi.getAll()
    oldDeviceList.value = res.data
  } catch(err) {
    message.error(`获取线路保护装置设备列表失败: ${err}`)
  }
}
...
const _getStatistics = async () => {
  ...
  try {
    const data = await lineProtectiveApi.getStatistics({ 
      ...searchForm.value,
...
const _getTableList = async () => {
  ...
  try {
    ...
    const data = await lineProtectiveApi.pageBySheet({ 
      ...searchForm.value,
      time: dayjs(searchForm.value.time).format("YYYY-MM-DD"),
      page: current, 
      size: pageSize
